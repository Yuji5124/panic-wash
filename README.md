...
`js/app.js` 冒頭の「バランス調整用定数」ブロックで変更できます。
...
`getDirtyInterval()` 関数で管理。`setInterval` ではなく `setTimeout` 再スケジュール方式。

#### 全消し攻撃

| 定数 | 値 | 説明 |
|---|---|---|
| `ZENKESHI_DIRTY` | 5 マス | 即撃ち（通常攻撃） |
| `ZENKESHI_DIRTY_CHARGE` | 7 マス | ため撃ち（強攻撃） |
| `ZENKESHI_CHARGE_MS` | 2000 ms | 強攻撃チャージ時間 |

全消し達成後 **2秒待つ** と「強ウォッシュ！」に昇格（橙色に変化）。その間に盤面が汚れるリスクがある。

#### ピンチ洗浄

| 定数 | 値 | 説明 |
|---|---|---|
| `PINCH_DIRTY_THRESHOLD` | 12 マス | 発動する汚れ数しきい値 |
| `PINCH_TIME_THRESHOLD` | 20 秒 | 発動する残り時間しきい値 |
| `PINCH_CHAIN_CHANCE` | 20% | タップ時に隣の汚れも消える確率 |

自分の汚れが 12 マス以上 **かつ** 残り 20 秒以下の場合に発動。タップで汚れを消すたびに 20% の確率で隣の汚れも 1 マス消える。
